...
export async function POST(request: Request) {
 const body = await request.json(); const raw = String(body.raw ?? "").trim();
 if (!raw) return NextResponse.json({ error: "Tell Suchi what outcome you want." }, { status: 400 });
 if (raw.length > 4000) return NextResponse.json({ error: "That dump is too long. Start with the outcome and Suchi will take it from there." }, { status: 413 });
 const words = raw.replace(/[.!?]+$/, "").split(/\s+/); const title = words.slice(0, 9).join(" ") + (words.length > 9 ? "…" : "");
 const domain: "Life" | "Work" = /family|home|personal|health|trip|house/i.test(raw) ? "Life" : "Work";
 const nextAction = "Suchi is turning your outcome into an execution plan."; const tasks = planMission(raw);
 try {
 ...
 } catch (error) { console.error("mission persistence failed", error); return NextResponse.json({ error: "Suchi could not save this mission." }, { status: 500 }); }
}
