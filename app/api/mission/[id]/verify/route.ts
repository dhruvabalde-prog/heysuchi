...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 const {data:tasks}=await supabase.from("mission_tasks").select("status,title").eq("mission_id",id);
 const state=missionProgress(tasks??[]); const next=(tasks??[]).find(t=>!["done","verified"].includes(t.status));
 await supabase.from("missions").update({progress:state.progress,status:state.complete?"done":"working",next_action:state.complete?"Mission complete.":next?"Suchi is ready for the next step.":"Suchi is working."}).eq("id",id).eq("owner_id",user.id);
