...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 if(error)return NextResponse.json({error:error.message},{status:500}); if(!data)return NextResponse.json({error:"Artifact not found."},{status:404});
 return new NextResponse(data.content,{headers:{"Content-Type":"text/markdown; charset=utf-8","Content-Disposition":`attachment; filename="${data.name||"mission.md"}"`}});
}
