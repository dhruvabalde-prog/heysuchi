...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const {data:mission,error}=await supabase.from("missions").select("*").eq("id",id).eq("owner_id",user.id).single(); if(error||!mission)return NextResponse.json({error:"Mission not found."},{status:404});
