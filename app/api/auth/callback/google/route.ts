...
export async function GET(request: Request) {
  ...
  try {
    const identity = await exchangeGoogleCode(code, callbackUrl(request));
    ...
    if (invite) {
      await supabase.from("platform_admin_members").upsert(
        { identity_id: existing.id, role_id: invite.role_id, granted_by: existing.id },
        { onConflict: "identity_id" },
      );
      await supabase.from("platform_admin_invites")
    ...
    if ((count ?? 0) === 0) {
      ...
      if (role) {
        await supabase.from("platform_admin_members")
          .insert({ identity_id: existing.id, role_id: role.id, granted_by: existing.id });
      }
    }
